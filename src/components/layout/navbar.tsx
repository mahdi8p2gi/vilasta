        ...
        <div className="flex items-center gap-1.5">
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenu}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="lg:hidden rounded-full" aria-label="منو">
              ...
              <div className="mt-6 flex flex-col gap-1">
                {navItems.map((item) => (
                  <button
                    ...
                    className={cn(
                      "rounded-xl px-4 py-3 text-right text-sm font-medium transition-colors hover:bg-accent",
                      view === item.view && "bg-accent text-accent-foreground"
                    )}
                  >
                    {item.label}
                ...
                {!user && (
                  <>
                    ...
                    <Button onClick={() => { openAuth("login"); setMobileMenu(false); }} className="w-full rounded-xl">
                      ورود
                    </Button>
                    <Button variant="outline" onClick={() => { openAuth("register"); setMobileMenu(false); }} className="w-full rounded-xl">
                      ثبت‌نام
                    </Button>
                  </>
                )}
